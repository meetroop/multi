...
public class AnalyserUtility {
     ...
     */
	 private static final Pattern RX_VERSION = Pattern.compile("\\d+(\\.\\d{1,6})+(\\.?([_-](release|beta|alpha|\\d+)[a-zA-Z_-]{1,3}\\d{0,8}|[a-zA-Z_-]{1,30}\\d{0,8}))?");
     ...
     */
    private static final Pattern RX_SINGLE_VERSION = Pattern.compile("\\d+(\\.?([_-](release|beta|alpha)|[a-zA-Z_-]{1,30}\\d{1,8}))?");
	...
	public List<Dependencies> distribute(String absoluteFileName) {
		...
		registeredAnalysers.keySet().forEach(p -> {
	...
	private void collatePomBom(Map<String, Dependencies> bom, JsonObject j) {
		...
		pDataArray.forEach(p -> {

			JsonArray jsonPomData = p.getAsJsonObject().get("data").getAsJsonArray();

			// generally executed 1 time only
			jsonPomData.forEach(pom -> {

				String fileLocation = pom.getAsJsonObject().get("artifactLocation").getAsString();

				JsonArray pomMembers = pom.getAsJsonObject().getAsJsonArray("members").getAsJsonArray();

				// generally executed multiple times
				pomMembers.forEach(ext -> {

//					JsonObject extraData = ext.getAsJsonObject().get("extraDatas").getAsJsonArray().get(0)
//							.getAsJsonObject();
	...
	private void collateJarBom(Map<String, Dependencies> bom, JsonObject j) {
		...
		jArray.forEach(jar -> {
			...
			String[] productAndVersion = getProductAndVersionFromFileName(jarObj.get("artifactFileName").getAsString());
			...
			if (jsonArrayCheckSUM != null && jsonArrayCheckSUM.size() > 0) {
				...
				final String[] jarName = jarObj.get("jarName").getAsString().split(ResourceConstants.NAME_SEPARATOR);
				...
				for(JsonElement je : jsonArrayCheckSUM) {
					...
					evdDataProd.setEvidenceName(jobj.getAsJsonObject().get("signatureType").getAsString());
					evdDataProd.setActualEvidenceValue(jobj.getAsJsonObject().get("signatureValue").getAsString());
					...
					evdDataVer.setActualEvidenceValue(jobj.getAsJsonObject().get("signatureValue").getAsString());
			...
			if (!isMatched) {
				JsonObject deviatedClasses = jarObj.get("deviatedClasses") == null ? null
				...
				if (deviatedClasses != null && deviatedClasses.size() > 0) {
					...
					deviatedClasses.keySet().forEach(dc -> {
	...
	private String getID(String group, String product) {
		...
		return (ResourceConstants.ID_PREFIX+group+ResourceConstants.NAME_SEPARATOR+product).toLowerCase();

	}

	private String createTEMPID(String availableData) {

		String tempID = (ResourceConstants.ID_PREFIX_TEMP + ResourceConstants.NAME_SEPARATOR + availableData)
				.toLowerCase();
	...
	public JsonObject callAnalyserAPI(String type, String absoluteFileName) {
		...
		try {
			...
			jObj.add("data", new JsonParser().parse("Exception occured while analysing the json for :" + type));
